...
- `pushObject` は**毎回新しいラッパを返す**。同一性は lhat の等値規則が持つ（hostdata の `=` は tag + 実体ポインタ）ので、別のゲッターが作った 2 つは等しくハッシュも一致する — `is^` だけが別。**ホスト側にキャッシュを置いてはいけない**: lhat の収集は漸進的で、マークで白と決まった包みを スイープ前に返すと、L^ が生きた場所へ入れた物がそのまま解放される（`lh.h` に経緯、[lhat-issues.md](docs/porting/lhat-issues.md) に報告）。Lua の弱値表が与えるのは「根にしない」ではなく「コレクタがマークの終わりにエントリを消す」保証で、ホストの map にはその口が無い。churn が問題になったら lhat 側の弱参照キャッシュ（提案済み）を待つ
