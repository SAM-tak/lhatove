...
**未実装。** M1 完了時点でこの形が動く想定。実装と食い違ったらこのファイルを直す。
...
yieldable な `p^` で、毎フレーム `yield^` する。終了は `return^`
（`nil^` / `number^` = 終了コード、`"restart"` = 再起動）。
...
public^let^ run = p^ {
    ...
    loop^ {
        love.event.pump()
        if^ love.event.dispatch(h) { return^ 0 }
        h.update(love.timer.step())
        love.graphics.origin(); love.graphics.clear()
        h.draw(); love.graphics.present()
        yield^
    }
}
```
...
- 真偽値以外を条件に使えない（`0` や `""` は false ではない）
- 多値戻りは固定幅タプル。`local x, y = obj:getPosition()` 相当は分解束縛
...
- 実行時のコード読み込みは `std.load.file(path)` / `std.load.text(src, name)`、または `love.filesystem.load`
