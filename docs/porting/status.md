...
| M6 | 仕上げ（nogame.lh・restart・Lua 残骸削除・testing/ 移植開始） | 引数なし起動で nogame 表示 | 進行中（2026-08-24）。nogame.lh（physics のチェーン + 雲 + ドロップで restart）、restart（`love.event.restart(payload)` → 再 boot → `love.event.restartValue()`、`testing/lh/restart` exit=10） |
